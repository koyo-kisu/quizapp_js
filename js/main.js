...

{
    ...
    const quizSet = shuffle([
        {q: '次のうちでペンギンがいない場所は?', c: ['北極', '南極', 'ガラパゴス諸島']},
        {q: 'ウミガメはなぜ出産の際に涙を流すの？', c: ['体の中にある余計な塩分を出しているから', '我が子に出会えて感動しているから', '目が乾燥して痛いから']},
        {q: 'カタツムリがコンクリートブロックの下によくいるのはなぜ？', c: ['食べているから', 'ひんやりして気持ちいから', '敵が襲ってこないから']},
        {q: 'ミツバチ1匹が一生で作れる蜂蜜の量は？', c: ['5グラム', '50グラム', '500グラム']},
    ]);
    ...
    function shuffle(arr) {
        for(let i = arr.length - 1; i > 0; i--) {                //配列の最後の要素[i]が0より大きい間、iを1ずつ減らすループ            
    ...
    function setQuiz() {
        ...
        //コンソールに表示されたデータもシャッフルされることがわかった→正解の選択肢がわからなくなる
        //スプレッド演算子→初期値の配列をそのまま渡す

        while (choices.firstChild) {        //choicesのはじめの要素がnullになるまでの間、最初の要素を取り除く
            choices.removeChild(choices.firstChild);
        }

        shuffledChoices.forEach(choice => {       //quizSetのcurrentNum番目のcを一つひとつ取り出してchoiceに代入
    ...
    next_btn.addEventListener('click', () => {
        if (next_btn.classList.contains('disabled')) {      //disableクラスを持っているなら以降の処理をしない
            return;
        }

        next_btn.classList.add('disabled');     //disabledクラスを追加

        if (currentNum === quizSet.length - 1) {        //最後の問題ならscoreを表示
